...
class project {
	...
	*
	*/
	public function getAll($project_authorid) {
		$entry_query = $this->db->query("SELECT COUNT(corr_user_id) AS clienti, project_description, idproject FROM tab_project LEFT JOIN tab_user_project ON idproject=corr_project_id WHERE project_authorid='".$project_authorid."' AND corr_isCollaborator='0' GROUP BY idproject, project_description");
		if ($entry_query->num_rows) {
			return $entry_query->rows;
		}
		else {
	...
	*/
	...
	public function getAllWithClients($user_id) {
		$entry_query = $this->db->query("SELECT * FROM tab_project AS tp LEFT JOIN tab_user_project AS tup ON idproject=corr_project_id LEFT JOIN tab_user ON corr_user_id=user_id WHERE (project_authorid='".$user_id."' OR corr_user_id='".$user_id."') AND corr_isCollaborator=0");
		if ($entry_query->num_rows) {
			return $entry_query->rows;
		}
		else {
