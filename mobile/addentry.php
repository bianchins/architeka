...
switch($request->post['action']) {
	 ...
	 */
	case "addAppointment":
		$appoint->insertAppointment($user->getId(),$session->data['idproject'],$request->post['appointment_description'],$request->post['appointment_place'],$request->post['appointment_date'],$request->post['appointment_time']);		
...
switch($request->get['type']) {
	...
	//Niente allegati nella versione mobile
	//case "4": $specificTemplate='entry/allegato.php'; break;
	//case "5": $specificTemplate='entry/mappa.tpl'; break;
	//case "6": $specificTemplate='entry/date.tpl'; break;
...
$content = $template->load(DIR_TEMPLATE_MOBILE . $specificTemplate);
